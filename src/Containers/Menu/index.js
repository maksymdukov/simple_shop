...
const Menu = ({ menu, loading, fetchFullMenu }) => {
    ...
    const onFilterClickHandler = filterName => {
        ...
        if (filterName === "all") {
            ...
            menu.order.forEach(
                filterItem => (updFilterState.filters[filterItem] = false)
            );
            ...
            updFilterState.activeCounter = filters.filters[filterName]
                ...
                : updFilterState.activeCounter + 1;
            updFilterState.filters[filterName] = !updFilterState.filters[
                filterName
            ];
    ...
    if ("all" in filters.filters) {
        menu.order.forEach(filterName => {
            if (filters.filters.all) {
                // "All" filter is On
                filteredList.push(
                    <FilteredItem key={filterName} {...{ menu, filterName }} />
                );
...
Menu.propTypes = {
    ...
    fetchFullMenu: PropTypes.func.isRequired
};
